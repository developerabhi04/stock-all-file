...
const sanitizePhoneNumber = (phoneNumber) =>
    String(phoneNumber || '').replace(/\D/g, '').replace(/^91/, '').slice(0, 10);

const sanitizeOTP = (otp) =>
    String(otp || '').replace(/\D/g, '').slice(0, 6);
...
const serializeParams = (params = {}) => JSON.stringify(stableSortObject(params));

const makeCacheKey = (key, params = {}) => `${CACHE_PREFIX}:${key}:${serializeParams(params)}`;
...
class ApiService {
        ...
        this.api.interceptors.response.use(
            ...
            async (error) => {
                ...
                const shouldRetry =
                    isGet &&
                    !config._noRetry &&
                    (!error.response || (status >= 500 && status < 600) || error.code === 'ECONNABORTED');
                ...
                if (shouldRetry) {
                    ...
                    if (config._retryCount < MAX_RETRIES) {
                        ...
                        console.log(`🔁 Retrying ${url} in ${backoff}ms (attempt ${config._retryCount}/${MAX_RETRIES})`);
                        await sleep(backoff);
    ...
    async cacheFirstRequest({
        ...
        const hasCachedData = cached && typeof cached.data !== 'undefined' && cached.data !== null;
        ...
        if (preferCache && hasCachedData) {
            if (backgroundRefresh || !fresh) {
                requestFn()
                    ...
                    .catch((error) => {
                        console.log(`ℹ️ Background refresh failed for ${cacheKey}:`, error?.message || error);
                    });
            }
    ...
    normalizeMarketIndex(data) {
        return {
            ...
            categoryId: data?.categoryId || data?.category?._id || data?.category || null,
            ...
            defaultDailyRate:
                data?.defaultDailyRate === null || typeof data?.defaultDailyRate === 'undefined'
                    ? null
                    : Number(data?.defaultDailyRate),
            minimumInvestment:
                data?.minimumInvestment === null || typeof data?.minimumInvestment === 'undefined'
                    ? null
    ...
    normalizeInvestmentPreview(data = {}) {
        return {
            ...
            effectiveDailyRate: Number(data?.effectiveDailyRate ?? data?.dailyRate ?? data?.rate ?? 0),
            dailyInterestAmount: Number(data?.dailyInterestAmount ?? data?.dailyEarning ?? data?.perDayReturn ?? 0),
            ...
            hasSufficientBalance: typeof data?.hasSufficientBalance === 'boolean' ? data.hasSufficientBalance : true,
            ...
            defaultDailyRate:
                data?.defaultDailyRate === null || typeof data?.defaultDailyRate === 'undefined'
                    ...
                    : Number(data?.defaultDailyRate),
            slabDailyRate:
                data?.slabDailyRate === null || typeof data?.slabDailyRate === 'undefined'
                    ? null
    ...
    normalizeInvestmentOrder(data = {}) {
        ...
        return {
            ...
            effectiveDailyRate: Number(data?.effectiveDailyRate ?? data?.dailyRate ?? data?.rate ?? 0),
            dailyInterestAmount: Number(data?.dailyInterestAmount ?? data?.dailyEarning ?? data?.perDayReturn ?? 0),
            totalInterestEarned: Number(data?.totalInterestEarned ?? data?.earned ?? 0),
    ...
    getMyOrdersEndpoint() {
        return ENDPOINTS.GET_MY_INVESTMENT_ORDERS || '/market/investments/my-orders';
    }
    ...
    async getPaymentConfig(options = {}) {
        try {
            ...
            const result = await this.cacheFirstRequest({
                ...
                preferCache: options.preferCache ?? true,
                backgroundRefresh: options.backgroundRefresh ?? true,
                requestFn: () => this.api.get(ENDPOINTS.GET_PAYMENT_CONFIG || '/payment/config'),
    ...
    async getFeaturedIndices(options = {}) {
        try {
            ...
            const result = await this.cacheFirstRequest({
                ...
                transform: (response) => {
                    const indices = response.data?.data?.indices || response.data?.data || [];
    ...
    async getMarketIndexDetails(indexId, options = {}) {
        try {
            ...
            const result = await this.cacheFirstRequest({
                ...
                backgroundRefresh: options.backgroundRefresh ?? true,
                requestFn: () =>
                    this.api.get(`${ENDPOINTS.GET_MARKET_INDEX_DETAILS_BY_ID}/${indexId}`),
    ...
    async getMarketIndexHistory(symbol, period = '1D', options = {}) {
        try {
            console.log(`📈 Fetching market index history for: ${symbol}, period: ${period}`);
            ...
            if (error?.response?.status === 404) {
                return { success: true, data: [], raw: null, message: 'Price history not found' };
            }
            return this.handleError(error);
        }
    }
    ...
    async getMarketIndexDailyHistory(symbol, page = 1, limit = 10, options = {}) {
        try {
            ...
            const result = await this.cacheFirstRequest({
                ...
                backgroundRefresh: options.backgroundRefresh ?? true,
                requestFn: () =>
                    this.api.get(`${ENDPOINTS.GET_MARKET_INDEX_DAILY_HISTORY}/${symbol}`, { params }),
                transform: (response) => ({
            ...
            if (error?.response?.status === 404) {
                return { success: true, data: [], raw: null, message: 'Daily history not found' };
            }
            return this.handleError(error);
        }
    }
    ...
    async getMarketInvestmentInfo(symbol, options = {}) {
        try {
            ...
            const result = await this.cacheFirstRequest({
                ...
                backgroundRefresh: options.backgroundRefresh ?? true,
                requestFn: () =>
                    this.api.get(`${ENDPOINTS.GET_MARKET_INVESTMENT_INFO}/${symbol}/investment-info`),
                transform: (response) => response.data?.data || null,
            });
    ...
    async getInvestmentPreview({ indexId, amount }) {
        try {
            ...
            console.log(`🧮 Fetching investment preview: index=${indexId}, amount=${amount}`);
            ...
            return {
                ...
                message: response.data?.message || 'Investment preview fetched successfully',
            };
        } catch (error) {
            return this.handleError(error);
        }
    ...
    async placeInvestmentOrder(payload) {
        try {
            ...
            console.log(`🛒 Placing investment order: index=${payload?.indexId}, amount=${payload?.amount}`);
            ...
            const responseData = response.data?.data || {};
            const order = responseData?.order || responseData?.investment || responseData;
    ...
    async saveFcmToken(fcmToken) {
        try {
            ...
            const response = await this.api.put('/user/fcm-token', { fcmToken }, { noRetry: true });
    ...
    async getUserNotifications() {
        try {
            const response = await this.api.get(ENDPOINTS.GET_USER_NOTIFICATIONS || '/notifications/user');
            return {
                success: true,
                data: response.data.data,
    ...
    async markNotificationAsRead(notificationId) {
        try {
            const response = await this.api.patch(
                `${ENDPOINTS.MARK_NOTIFICATION_AS_READ || '/notifications/user'}/${notificationId}/read`
            );
            return {
                success: true,
                data: response.data.data,
    ...
    async markNotificationAsClicked(notificationId) {
        try {
            const response = await this.api.patch(
                `${ENDPOINTS.MARK_NOTIFICATION_AS_CLICKED || '/notifications/user'}/${notificationId}/click`
            );
            return {
                success: true,
                data: response.data.data,
    ...
    async getMyReferralInfo() {
        try {
            ...
            const response = await this.api.get(
                ENDPOINTS.GET_MY_REFERRAL_INFO
            );
    ...
    async getMyReferralHistory(page = 1, limit = 20) {
        try {
            ...
            const response = await this.api.get(
                ENDPOINTS.GET_MY_REFERRAL_HISTORY,
                {
                    params: {
                        page,
                        limit,
