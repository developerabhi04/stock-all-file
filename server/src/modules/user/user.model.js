...
userSchema.pre("validate", function (next) {
  if (!this.referralCode && this._id) {
    this.referralCode = `TH${this._id.toString().slice(-8).toUpperCase()}`;
  }
...
userSchema.methods.canTrade = function () {
  return this.isVerified && Number(this.walletBalance || 0) >= 10;
};
...
userSchema.methods.getPrimaryBankAccount = function () {
  return (
    this.bankAccounts.find((account) => account.isPrimary) ||
    this.bankAccounts[0]
  );
};
