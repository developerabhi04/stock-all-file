...
export const maybeRewardReferralOnRechargeService = async ({
  ...
  if (!transactionId) {
    throw new ApiError(400, "Source recharge transaction ID is required");
  }
  ...
  const refereeBalanceAfter = refereeBalanceBefore + REFERRAL_BONUS_AMOUNT;

  const referrerBalanceAfter = referrerBalanceBefore + REFERRAL_BONUS_AMOUNT;
    ...
    [
      {
        ...
        status: "completed",
        description: "Referral bonus for first approved recharge",
        metadata: {
...
export const getMyReferralInfoService = async ({ userId }) => {
  ...
  const websiteUrl = (
    process.env.WEBSITE_URL || "https://tradehub-website-two.vercel.app"
  ).replace(/\/$/, "");
  ...
  return {
    ...
    totalInvited,
    totalRewarded,
    totalPending: Math.max(totalInvited - totalRewarded, 0),
...
export const getMyReferralHistoryService = async ({
  ...
  const [referrals, total] = await Promise.all([
    Referral.find(filter)
      .populate("referee", "fullName phoneNumber createdAt")
      .sort({ createdAt: -1 })
...
export const getAllReferralsAdminService = async ({
  ...
  const [referrals, total, summary] = await Promise.all([
    Referral.find(filter)
      .populate("referrer", "fullName phoneNumber referralCode")
      .populate("referee", "fullName phoneNumber createdAt")
      .sort({ createdAt: -1 })
      .skip((pageNum - 1) * limitNum)
      .limit(limitNum)
...
export const getReferralStatsAdminService = async () => {
  const [totalReferrals, totalRewarded, totalPending, totalPaidOut] =
    await Promise.all([
      Referral.countDocuments(),
  ...
  return {
    ...
    totalRewarded,
    totalPending,
    totalPaidOut: (totalPaidOut?.[0]?.total || 0) * 2,
  };
};
