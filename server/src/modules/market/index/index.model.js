...
indexSchema.pre("validate", function (next) {
  ...
  if (
    [this.currentValue, this.highValue, this.lowValue, this.previousClose].some(
      Number.isNaN,
    )
  ) {
    return next(
      new Error(
        "Current value, high value, low value, and previous close must be valid numbers",
      ),
    );
  }
  ...
  if (
    ...
    typeof this.minimumInvestment !== "undefined"
  ) {
  ...
  if (
    ...
    typeof this.lockPeriodDays !== "undefined"
  ) {
  ...
  this.changePercent =
    this.previousClose > 0
      ? Number(
          (
            ((this.currentValue - this.previousClose) / this.previousClose) *
            100
          ).toFixed(2),
        )
      : 0;
