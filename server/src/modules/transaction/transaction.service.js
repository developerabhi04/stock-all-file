...
import { maybeRewardReferralOnRechargeService } from "../referral/referral.service.js";
...
export const approvePaymentService = async ({
  ...
  try {
    ...
    transaction.adminAction = {
      ...
      actionBy: adminId,
      actionAt: new Date(),
      reason: verificationNote || "Payment verified",
    };

    transaction.description = `Wallet credited via ${
      transaction.paymentDetails.gateway || "manual payment"
    }. Verified by admin.`;
     ...
     */
    referralResult = await maybeRewardReferralOnRechargeService({
  ...
  await sendNotificationSafely({
    ...
    message: `Your deposit of ₹${Number(transaction.amount).toLocaleString(
      "en-IN",
  ...
  if (referralResult) {
    await sendNotificationSafely({
      ...
      adminId,
      data: {
        referralId: referralResult.referralId.toString(),
        amount: String(referralResult.rewardAmount),
        role: "referee",
      },
    });
...
export const approveWithdrawalService = async ({
  ...
  transaction.withdrawalDetails.utrNumber = String(utrNumber).trim();
  ...
  await sendNotificationSafely({
    ...
    message: `Your withdrawal of ₹${Number(transaction.amount).toLocaleString(
      "en-IN",
...
export const rejectWithdrawalService = async ({
  ...
  try {
    ...
    user.walletBalance = balanceBefore + Number(transaction.amount);
...
export const getAllTransactionsAdminService = async ({
  ...
  // Filter transactions for one specific user when userId is passed.
  if (userId) {
  ...
  // 1000 works from UserDetails.jsx.
  // You can set a different safe cap later if required.
...
export const getWithdrawalStatsAdminService = async () => {
  ...
  return {
    ...
    grandTotal:
      (completed[0]?.totalAmount || 0) + (pending[0]?.totalAmount || 0),
  };
};
